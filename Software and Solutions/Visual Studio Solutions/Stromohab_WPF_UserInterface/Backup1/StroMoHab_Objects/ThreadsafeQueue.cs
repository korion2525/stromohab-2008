...
namespace StroMoHab_Objects
{
    ...
    public class ThreadsafeQueue<T> : IDisposable
    {
        ...
        private int NUMBER_OF_ITEMS_TO_LOCK_ENQUEUING_AT;
        ...
        public ThreadsafeQueue(int capacity)
        {
            ...
            NUMBER_OF_ITEMS_TO_LOCK_ENQUEUING_AT = (int)((double)MAX_NUMBER_OF_ITEMS_IN_QUEUE * 0.8); //NUMBER_OF_ITEMS_TO_LOCK_ENQUEUING_AT currently set to 4/5 total queue size.
        ...
        public bool Enqueue(T data)
        {
            ...
            if (_queue.Count < NUMBER_OF_ITEMS_TO_LOCK_ENQUEUING_AT)
            {
                _lock = false;
            }

            //if not accessing the queue
            if (_isBeingAccessed == false && _lock == false)
            {
